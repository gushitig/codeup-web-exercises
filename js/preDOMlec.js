(function() {
    ...
    const myBoxes = document
    .getElementsByClassName("myBox")
    console.log(myBoxes);
    for (let i = 0; i < myBoxes.length; i++) {
        myBoxes[i].style.color = "red";
    }
        //bootstrap overrode the color change here
        //going to return an array even if only one element is returned
...
*/
    ...
    myLIs[1].innerHTML = "<b>Item 2</b>"; //WILL BE PROCESSED AS HTML BUT INNERTEXT WILL NOT

    // get access to the forms object and log it
    // log forms.login
    //  point out the 2 fields and the button that are children of it
    console.log(document.forms);
    console.log(document.forms.my_form[0]); //use id and query selectors instead
    ...
    console.log(userName.getAttribute("value")); //this value returns the default value, not the current value
    console.log(userName.value); //property value will return the current value, not the getAttribute
    console.log(userName.defaultValue);

    // use setAttribute to set the value attribute for username
    userName.setAttribute("value", "hi there"); //sets DEFAULT VALUE not value
    console.log(userName.value);
    console.log(userName.defaultValue);
    ...
    //  explain importance of data-id CONNECTS THE FRONT END TO THE BACK END
